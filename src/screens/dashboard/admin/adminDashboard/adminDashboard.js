...
const AdminDashboard = () => {
  ...
  const loadItems = async () => {
    setIsLoading(true);
    try {
      const response = await api.get(`https://${NGROK_URL}/projects/countPeople`, {});
      ...
      <div className="admin-child">
        ...
        <div className="dashboard-button">
          ...
          <div className="button-container">
            {
              <div>
                <button className="ui button" data-testid="create" onClick={createOnclick}>
                  Create Project
                </button>
                  ...
                  {currentPageData.length === 0 ? (
                    ...
                    currentPageData.map((item, index) => (
                      <tr key={item.projectName}>
                        ...
                        <td>
                          {" "}
                          <a href={`/ProjectDetails/${item.projectId}/${item.projectName}`}>
                            {item?.projectName || "N/A"}
                          </a>
                        </td>
                        <td>{item?.countPeople || "0"}</td>
                      </tr>
                    ))
                  )}
                </tbody>
