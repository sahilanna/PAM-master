...
const ProjectAnalytics = () => {
  ...
  const fetchData = async () => {
    try {
      const activeResponse = await api.get(`https://${NGROK_URL}/projects/count/active`);

      const inactiveResponse = await api.get(`https://${NGROK_URL}/projects/count/inactive`);
  ...
  const handleDownloadCSV = () => {
    ...
    const csvContent =
      "data:text/csv;charset=utf-8," + csvData.map((e) => Object.values(e).join(",")).join("\n");
