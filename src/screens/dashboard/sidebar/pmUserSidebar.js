      ...
      <Sidebar as={Menu} animation="overlay" visible vertical inverted style={{ textAlign: 'left' }}>
        <Menu.Item className="custom-menu-item">
          <a href="/adminDashboard" className="text-decoration-none" style={{ color: 'inherit' }}>
            <span style={{ marginRight: '90px', fontSize: '30px' }}>{title}</span>
          </a>
        </Menu.Item>
        <br />
        <br />
        {links.map((link) => (
          <NavLink key={link.to} className='text-style' exact to={link.to} activeClassName="activeClicked">
            <div style={{ display: 'flex', alignItems: 'center' }}>
              <span style={{ marginRight: '10px' }}><Icon name={link.icon} /></span>
              <span>{link.text}</span>
            </div>
          </NavLink>
        ))}
